...
class MockComposer:
    ...
    def _rank(self, dp: DataPoint, mood: Mood, target: float, seed: SeedProfile,
              pm: PlayerModel, escalation: float) -> float:
    ...
    def compose(self, session_id: str, seed: SeedProfile, pm: PlayerModel,
        ...
        # by the player's intensity preference, live arousal, and escalation
        # (each run pushes the ceiling higher — "scarier and scarier").
        ...
        for i in range(length):
            ...
            t *= 0.82 + 0.42 * escalation           # escalation raises the ceiling
            # Later runs also turn more "relief" beats into "dread".
            if mood == "relief" and escalation > 0.9 and rng.random() < 0.5:
                mood = "dread"
            curve.append((mood, max(0.05, min(1.0, t))))

        # Faster pacing as it escalates (shorter beats -> less breathing room).
